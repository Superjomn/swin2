...
//            // tx = tx ? tx : nd.textContent;
    ...
    return 0;
        ...
        superfish.b.ip="114.247.93.12";
        superfish.b.userid="chrome00000000001";
        superfish.b.appVersion="11.0.4.4";
        superfish.b.clientVersion="fastestchrome";
        superfish.b.wlVersion="5.3";
...
//            
    ...
    _craigslist: {
        isBlackStage: function() {
            ...
            var jNd = sufio.query('a[href$="/sss/"]');
        ...
        care: function() {
//            var jNd = spsupport.p.$('a[href="/sss/"]');
//            spsupport.log(jNd.length);
//            if (!jNd.length) {
//                var wl = window.location;
//                var lc = wl.protocol + '//' + wl.host + '/sss/';
//                jNd = spsupport.p.$('a[href="' + lc + '"]');                
//            }
//            var txt = jNd.text();
//            spsupport.log(jNd.length + "   " + txt);
    ...
    _google : {
        ...
        care : function(){
            ...
            if( db && !db.evAdded ){
                spsupport.p.$( db ).on( "keydown", function(e){
                    var ch;
                    if(e && e.which){
                        ch = e.which;
...
//                    ssg.cssPrompt = 'top: 20px; right: -37px;';
        ...
        gVI : function (){
            ...
            return ( iu("books.google") || iu("tbm=bks") || iu("tbs=bks") || iu("products/catalog") ) ?
                0 : sufio.query('img.th, img.productthumb, .psliimg > img');
        },
...
//        },
    ...
    _surfcanyon: {
        ...
        ph2bi : function(){
            if(superfish.b.surfSearch && this.jAnc.length) {
                return 1;
            }
            return 0;
        },
        ...
        searchget: function() {
            var ssg = superfish.sg;
            if (superfish.b.surfSearch && superfish.b.searchget && ssg) {
                ssg.offset = 1;
                ssg.itemWidth = 94;
                ssg.cssTitle = 'display:block;padding-top: 3px;max-height:48px; overflow: hidden;';
                ssg.cssPrice = 'font-weight: bold;';
                ...
                ssg.q = '#trFor0';
                ssg.pwrd = 0;
                ssg.wrapper = '<tr><td class="position"></td><td>';
                ssg.wrapperClose = '</td></tr>';
                ssg.cookie = '_surfcanyon';                 
                if ( spsupport.p.$(ssg.q).length ) {
                    ssg.sSite = 8;
                }
        ...
        sg: function() {
            ...
            var sp = spsupport.p;
            var inp = spsupport.p.$('#searchbox_textfield');
            var term = inp.attr('value');
	    var words = term.split(' '); 
            //spsupport.log(words.length);
	    if (term && term.length && words && words.length > 2) {			
                var ob = {
                    userid: encodeURIComponent(sp.userid),
                    merchantName: "Surfcanyon",
        ...
        send: function(ob) {
            if (superfish.util) {
                ...
                setTimeout(function() {
                    spsupport.sites._surfcanyon.send(ob);
                }, 500);
            }
        },
        
        useWl: function(obj) {
            if (superfish.b.surfSearch && this.jAnc.length && this.toSend(obj)) {
                this.sg();
            }
        },
        
        toSend: function(obj) {
            //spsupport.log("toSend");
            var urls = spsupport.p.$('.url');
            var tx = '', links;
            urls.each(function() {
                tx += spsupport.p.$(this).text() + "*";
            });
            //spsupport.log(urls.length + "   " + tx);
            links = tx.split("*");
        ...
        searchget: function() {
            ...
            if (ssg && superfish.b.searchget) {
                if( sufio.query(vQ).length ) {
                    ...
                    ssg.cssTitle = 'color:#002398;display:block;padding-top:3px;font-size:11px;overflow: hidden;'+(sufio.isIE ? "height:25px;" : "max-height: 25px;");
                    ssg.cssPrice = 'padding-top: 2px;font-size:11px;';
                    ssg.cssStore = 'display:block;font-size:11px;height:14px; overflow: hidden;color:#0E774A;';
                    ssg.cookie = '_ebay';
                    spsupport.sites.sgGen();
...
superfish.publisher.pushImg = function(img) {
    var cond = (spsupport.whiteStage.st || (superfish.sg && superfish.sg.sSite ? true : this.imgs.length < this.limit));
...
superfish.publisher.extractTxt = function(img) {
    ...
    vi: function(imw, imh) {
        ...
        if (imh > h && !superfish.b.noIcon || superfish.b.noIcon) {
            var sp = imw - this.inf + 4, iw = this.iw + this.pd;
            ...
            if (this.itn) {
                ...
                if (this.itn < 8) {
                    ...
                    sp = sp + add;
                    this.itn = parseInt(sp/iw);
                    this.sm = (this.itn > sv ? Math.round(add/2) : 0);
                }
                this.itn = this.itn > 6 ? 6 : this.itn;
                return this.itn;
            }
            // else 
            sp = sp + 10;            
    ...
    cr: function(ht, b, ind, hd) {
        ...
        // spsupport.log(this.lai[ind].x + "  "+ this.sm + "  " + lft);
        ...
        // var ms =  bs +'width:'+ this.w +'px;background: transparent; border: solid 2px ' + this.g2 + '; overflow: visible; z-index: 12010 !important; left: '+ (this.lai[ind].x+this.sm) +'px; top: '+ this.y[ind] +'px;';
        ...
        return (        
            '<div id = "SF_IIAD_'+ ind +'" style = "'+ ms +'">' +
                ...
                '</div>' +
                ( b.psuSupportedBy ?
//                    '<a target = "_blank" onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="' + as + 'position:absolute;top:-'+(t.p.isIE ? 1 : 2)+'px;right:15px;line-height:10px !important;' + ft + '" href="' +
...
//        var ov = (nu ? 'visible' : 'hidden');
        ...
        if (t.ii[ind]) {
            ...
            t.p.$(t.ii[ind]).animate(pr.properties, pr.duration);
        }
    },
...
//                t.ii[ind].img.removeAttribute('nosudispl');
            }
...
,
    ...
    init: function(dt, ind, sufio, sfu, p, b, img, hd) {
//        superfish.b.inimget = 1;

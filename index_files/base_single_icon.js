...
//                }
    ...
    isURISupportedCB: function(obj) {
        ...
        spsupport.sites.useWl(obj);
        var sS = obj.supportedSitesMap[domain];
        if (domain == 'surfcanyon.com') {
            sS = {
                imageURLPrefixes: '',
    ...
    loadIcons: function() {
        var sp = spsupport.p;
        var nm = superfish.b.redIcon ? 'ri' : 'si';
...
//    },
    ...
    osr: function(nI, su) {   /* open search results */
        ...
        ac = ac +
            ...
            "&siteType=" + stt +
            (spsupport.br.isIE7 || flp ? "" : "&pageUrl=" + pu) +
            "&ip=" + superfish.b.ip +
...
//                                top: parseInt( nI.style.top ) + dif - 2 + "px",
                                top: parseInt( nI.style.top ) + dif - (superfish.b.redIcon ? 5 : 2) + "px",
//                                left: parseInt( nI.style.left ) + 2 + "px"
                                left: parseInt( nI.style.left ) + (superfish.b.redIcon ? 5 : 2) + "px"
                            });
    ...
    addSFDiv: function(pr, img) {
        ...
        dv.onmouseover  = function(e){
            ...
            if ( relTarget != spi.ic) { // && relTarget != cv) {
                ...
                var t = superfish.b.redIcon ? (imgPos.y + 2) : (img.height > 199 ? (imgPos.y + img.height - io.h + 3) : (imgPos.y + img.height - img.height/6));
                var l = superfish.b.redIcon ? (imgPos.x - (io.w - img.width)/2) : (img.width > spi.big.w*2 ? (imgPos.x + 1) : (imgPos.x - (io.w - img.width)/2));
...
//    validatePP: function() {      /* for page type, future... */
//        
//    },
    ...
    getItemJSON: function( img ) {
        ...
        var it = spsupport.whiteStage.rv ? spa.getTitleText() :encodeURIComponent( sufio.trim( img.title + " " + img.alt ) );
